...
namespace Boids {
    class Obstacles : public Engine::Object {
        ...
        Engine::Cone tower = Engine::Cone(Engine::Vec<3>::origin, { 0.0, 0.0, 5.0 }, 0.5, 0.0, &this->tower_color);
        Engine::Rectangle2D ground = Engine::Rectangle2D(Engine::Vec<3>::origin, 5000.0, 5000.0, Engine::Quaternion::identity, &this->ground_color);
        Engine::Object
            tower_obj = Engine::Object(Engine::Vec<3>::origin, Engine::Quaternion::identity, true, &this->tower),
            ground_obj = Engine::Object({ -2500.0, 2500.0, 0.0 }, Engine::Quaternion::identity, true, &this->ground);

    public:

        Obstacles (const unsigned &number) :
            Engine::Object(Engine::Vec<3>::origin, Engine::Quaternion::identity)
        {
            ...
            std::uniform_real_distribution<float_max_t> random_xy(5.0, 200.0), random_z(0.0, 30.0), random_radius(0.5, 1.5), random_angle(-Engine::DEG180, Engine::DEG180);
            ...
            for (unsigned i = 0; i < number; ++i) {
                ...
                Engine::Vec<3> position = { std::cos(pos_angle) * pos_radius, std::sin(pos_angle) * pos_radius, random_z(random_generator) };
                ...
                switch (random_type(random_generator)) {
                    ...
                    break;
                    default:
                        mesh = new Engine::Cone(position, position + Engine::Vec<3>::random(0.5, 3.0), random_radius(random_generator), random_radius(random_generator) - random_radius.min(), bg);
                    break;
                }
